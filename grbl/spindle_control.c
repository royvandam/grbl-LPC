...
spindle_init() {
    ...
    pwm_init(&SPINDLE_PWM_CHANNEL, SPINDLE_PWM_USE_PRIMARY_PIN, SPINDLE_PWM_USE_SECONDARY_PIN,
             spindle_pwm_period, 0);
    pwm_enable(&SPINDLE_PWM_CHANNEL);

    pwm_gradient =
        (spindle_pwm_max_value - spindle_pwm_min_value) / (settings.rpm_max - settings.rpm_min);
#endif
...
uint8_t
spindle_get_state() {
    return spindle_get_enabled() ? spindle_get_direction() : SPINDLE_STATE_DISABLE;
}
...
void
spindle_set_speed(uint32_t pwm_value) {
...
#else
void
_spindle_set_state(uint8_t state)
#endif
{
    // Block during abort.
    if (sys.abort) {
    ...
    if (state == SPINDLE_STATE_DISABLE) {
        ...
        spindle::direction.set(state == SPINDLE_ENABLE_CCW);

        // Set spindle enabled
        spindle::enable.set(true);
        leds[2].set(true);
    }
...
spindle_sync(uint8_t state, float rpm) {
    ...
    protocol_buffer_synchronize();  // Empty planner buffer to ensure spindle is set when
                                    // programmed.
    spindle_set_state(state, rpm);
}
#else
void
_spindle_sync(uint8_t state) {
    if (sys.state == STATE_CHECK_MODE) {
        return;
    }
    protocol_buffer_synchronize();  // Empty planner buffer to ensure spindle is set when
